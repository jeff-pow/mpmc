...
if [ "$1" = "debug" ]; then
    cmake -DQM_ROTATION=OFF -DVDW=ON -DMPI=OFF -DCUDA=OFF -DCMAKE_BUILD_TYPE=Debug -Wno-dev ../
elif [ "$1" = "debugcuda" ]; then
    cmake -DQM_ROTATION=OFF -DVDW=ON -DMPI=OFF -DCUDA=ON -DCMAKE_BUILD_TYPE=Debug -Wno-dev ../
elif [ "$1" = "cuda" ]; then
