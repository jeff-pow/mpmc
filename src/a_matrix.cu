...
extern "C" {
#include "structs.h"
double *init_A_matrix(void *systemptr) {
    ...
    for (int i = 0; i < 3; i++) {
        for (int j = 0; j < 3; j++) {
            host_basis[i * 3 + j] = system->pbc->basis[j][i];
            host_recip_basis[i * 3 + j] = system->pbc->reciprocal_basis[j][i];
        }
    }
    ...
    (cudaMemcpy(device_omegas, host_omegas, N * sizeof(double), cudaMemcpyHostToDevice));
    cudaDeviceSynchronize();
